...
describe('regulatory authority boundaries', () => {
  ...
  function caseService(caseRow: any = null, referralRow: any = null) {
    ...
    const teamService = {
      ...
      isLeaderOfAny: jest.fn().mockResolvedValue(false),
      isLeaderOfAnyIds: jest.fn().mockResolvedValue(false),
      teamIdsForUser: jest.fn().mockResolvedValue([]),
      teamNamesForUser: jest.fn().mockResolvedValue([]),
    } as any;
    const service = new RegulatoryCaseService(
      cases,
      events,
      {} as any,
      {} as any,
    ...
    return { service, cases, referrals, authorities, teamService };
  }
  ...
  it('filters a case lookup by the operating authority at the database boundary', async () => {
    ...
    expect(cases.findOne).toHaveBeenCalledWith({
      where: { id: 77, leadAuthority: { id: authorityA.id } },
      relations: { assignedTeamRef: true },
    });
  });

  it('refuses to assign a team that the owning authority did not configure', async () => {
    const caseRow = { id: 77, leadAuthority: authorityA, assignedTeam: null, assignedTeamRef: null };
    const { service } = caseService(caseRow);
    await expect(service.assignTeam(77, actorA, authorityA, { team: 'Team B' })).rejects.toBeInstanceOf(TraceabilityRuleException);
  });
  ...
  it('moves ownership only when the receiving authority accepts and clears old internal assignment', async () => {
    const caseRow = {
      id: 77,
      status: RegulatoryCaseStatus.OPEN,
      leadAuthority: authorityA,
      assignedTo: { id: 3 },
      assignedTeam: 'Team A',
      assignedTeamRef: { id: 9 },
    };
    ...
    expect(caseRow.assignedTeamRef).toBeNull();
...
describe('regulatory oversight boundaries', () => {
  it('returns aggregate metrics only, never cases, evidence, or officers', async () => {
    const scope = {
      mode: 'OBSERVE',
      authority: { id: 10, code: 'A', name: 'Authority A', referralResponseDays: null },
    };
    ...
    const service = new RegulatoryOversightService(
      scopes,
      {} as any,
      {} as any,
      cases,
    ...
    expect(summary.authorities[0]).toEqual(
      expect.objectContaining({
        authority: { id: 10, code: 'A', name: 'Authority A' },
        open: 2,
        mode: 'OBSERVE',
      }),
    );
    expect(summary.canSupervise).toBe(false);
